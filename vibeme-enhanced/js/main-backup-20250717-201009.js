...
const VibeMe = {
    ...
    setupMatrixEffect: function() {
        ...
        const createBinaryColumn = () => {
            ...
            // Create individual character spans for vertical stacking
            const length = 20 + Math.floor(Math.random() * 10); // 20-30 characters
            for (let i = 0; i < length; i++) {
                const char = Math.random() > 0.5 ? '1' : '0';
                const charSpan = document.createElement('span');
                charSpan.className = 'matrix-char';
                charSpan.textContent = char;
                charSpan.style.opacity = Math.max(0.3, 1 - (i / length)); // Trailing effect
            ...
            setTimeout(() => column.classList.add('visible'), 100);
