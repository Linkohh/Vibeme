...
const VibeMe = {
    ...
    setupMatrixEffect: function() {
        console.log('🌌 MAIN.JS: Setting up matrix effect...');
        
        let matrixBg = document.getElementById('matrix-bg');
        
        // Recreate missing matrix background element if needed
        if (!matrixBg) {
            console.log('🔧 MAIN.JS: Creating missing matrix-bg element');
        ...
        console.log('✅ MAIN.JS: Matrix background element ready');

        const createBinaryColumn = () => {
            console.log('🔨 MAIN.JS: Creating binary column...');
            
            const column = document.createElement('div');
            column.className = 'binary-column';
            
            // Create longer, more visible content
            const chars = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
            const length = 80 + Math.floor(Math.random() * 40); // 80-120 characters
            let content = '';
            ...
            console.log('📍 MAIN.JS: Column positioned at:', column.style.left);
            console.log('🎬 MAIN.JS: Animation duration:', column.style.animationDuration);
            
            if (Math.random() > 0.7) {
                column.classList.add('fast');
                console.log('⚡ MAIN.JS: Fast column created');
            } else if (Math.random() < 0.3) {
                column.classList.add('slow');
                console.log('🐌 MAIN.JS: Slow column created');
            }
            
            matrixBg.appendChild(column);
            console.log('✅ MAIN.JS: Column added to DOM');
            
            setTimeout(() => {
                column.classList.add('visible');
                console.log('👁️ MAIN.JS: Column made visible');
            }, 100);
            
            column.addEventListener('animationend', () => {
                console.log('🔄 MAIN.JS: Column animation ended, removing');
        ...
        setInterval(() => {
            if (this.state.effectsEnabled && Math.random() > 0.7) {
                createBinaryColumn();
            }
        }, 2000);
    },
